    ...
    suspend fun login(email: String, password: String): ResourceState<User> {
        return try {
            ...
            if (firebaseUser != null) {
                ...
                val userDoc = db.collectionGroup(FirebaseConfig.Collections.USERS)
                ...
                if (userProfile == null) {
                    ...
                    db.collection(FirebaseConfig.getUsersPath(TradeFitApi.activeCompanyId))
                        .document(firebaseUser.uid).set(userProfile).await()
                } else {
                    if (userProfile.companyId.isNotBlank()) {
                        TradeFitApi.activeCompanyId = userProfile.companyId
                    }
    ...
    suspend fun register(email: String, password: String, name: String, role: String, companyId: String = ""): ResourceState<User> {
        return try {
            ...
            if (firebaseUser != null) {
                ...
                db.collection(FirebaseConfig.getUsersPath(companyId.ifBlank { TradeFitApi.activeCompanyId }))
